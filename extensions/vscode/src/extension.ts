...
export function activate(context: vscode.ExtensionContext) {
    ...
    let initDisposable = vscode.commands.registerCommand('aisp.initSession', () => {
        ...
        exec(`python "${pythonScript}" init "${workspacePath}"`, (error, stdout, stderr) => {
            if (error) {
    ...
    let generateDisposable = vscode.commands.registerCommand('aisp.generateResume', async () => {
        ...
        let mode = "basic";
        if (selection.label.includes('Zero-Token')) mode = "ast";
        if (selection.label.includes('Full Code')) mode = "full";
        if (selection.label.includes('Export ZIP')) mode = "zip";

        vscode.window.showInformationMessage(`Executing ${selection.label}...`);
        const pythonScript = path.join(PYTHON_CORE_DIR, "resume.py");
        
        exec(`python "${pythonScript}" ${mode} "${workspacePath}"`, (error, stdout, stderr) => {
            if (error) {
    ...
    let setTaskDisposable = vscode.commands.registerCommand('aisp.setActiveTask', async () => {
        ...
        if (!taskDesc) return; // User cancelled

        const pythonScript = path.join(PYTHON_CORE_DIR, "session.py");
        exec(`python "${pythonScript}" task "${workspacePath}" "${taskDesc}"`, (error, stdout, stderr) => {
            if (error) {
    ...
    vscode.workspace.onDidSaveTextDocument((document: vscode.TextDocument) => {
        ...
        if (fs.existsSync(aispDir)) {
            ...
            exec(`python "${pythonScript}" save "${workspacePath}" "${relativePath}"`, (error, stdout, stderr) => {
                if (error) {
    ...
    vscode.window.onDidChangeActiveTextEditor(editor => {
        if (editor) updateEditorState(editor);
    });
    ...
    function updateEditorState(editor: vscode.TextEditor) {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders) return;
        const workspacePath = workspaceFolders[0].uri.fsPath;
        const aispDir = path.join(workspacePath, '.ai-session');
